...
const validateUserID = (req, res, next) => {
  knex('dry').where('id', req.params.id).then(([data]) => {
    if (!data) {
      return res.status(400).json({
        error: {
          message: `Dry ID ${req.params.id} not found`
        }
      })
    }
    next()
...
/* GET all Dry fly fly record */
router.get('/', (req, res, next) => {
  knex('dry').then(data => res.status(200).json(data)).catch(err => next(err))
})

/* GET single Dry fly fly record */
router.get('/:id', validateUserID, (req, res, next) => {
  knex('dry').where('id', req.params.id).then(([data]) => res.status(200).json(data)).catch(err => next(err))
})

/* POST new Dry fly fly record */
router.post('/', validatePostBody, (req, res, next) => {
  const {id, name} = req.body

  knex('dry').insert({id, name}).returning('*').then(([data]) => res.status(201).json(data)).catch(err => next(err))
})

/* PATCH specified Dry fly fly record */
router.patch('/:id', validateUserID, buildPatchReq, (req, res, next) => {
  const {patchReq} = req

  knex('dry').where('id', req.params.id).first().update(patchReq).returning('*').then(([data]) => {
    res.status(200).json(data)
  }).catch(err => next(err))
})

/* DELETE specified Dry fly fly record */
router.delete('/:id', validateUserID, (req, res, next) => {
  knex('dry').where('id', req.params.id).first().del().returning('*').then(([data]) => {
